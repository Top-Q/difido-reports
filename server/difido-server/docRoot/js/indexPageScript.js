		...
		.DataTable(
			{
				...
				"fnCreatedRow": function (nRow, aData, iDataIndex) {
					...
					// Now we will change the style of each row according to the status of the results.
					if (titles.indexOf("# Failed") >= 0 && Number(aData[titles.indexOf("# Failed")]) > 0) {
						$(nRow).addClass('failure');
					} else if (titles.indexOf("# Warnings") >= 0 && Number(aData[titles.indexOf("# Warnings")]) > 0) {
						$(nRow).addClass('warning');
					} else if (titles.indexOf("# Tests") >= 0 && Number(aData[titles.indexOf("# Tests")]) > 0) {
						$(nRow).addClass('success');
					}
				},
				"drawCallback": function (settings) {
					// We collect the columns names so it will be easier for us later on to check the values of the
					// cells even if they will be filtered out or with different order.
					settings.aoColumns.forEach((col) => { titles.push(col.sTitle) });
